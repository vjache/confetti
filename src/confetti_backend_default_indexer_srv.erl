...
init([]) ->
    ...
    lager:log(notice, [], 
	      "Root configuration directory: '~s' (~s)", 
	      [ConfigDir,
...
handle_info({parse_source_file, 
	     SrcFile = #source_file{ file = File } }, 
	    #state{config_dir=Dir}=State) ->
...
json_decode_lined(Body0) ->
    Body = trim_right(Body0),
    Lines = binary:split(Body, [<<"\n">>, <<"\r">>], [global]),
    json_decode_lined(Lines, {incomplete, fun jsx:decode/1}, 0).

json_decode_lined(Empty, Result, _N) when Empty == [] orelse Empty == [<<>>] ->
    case Result of
